...
# ⬇️ 데이터와 토크나이저 다운로드
download_file(
    'https://huggingface.co/datasets/Yuchan5386/gpt_pretrain_small/resolve/main/output.jsonl?download=true',
    'dataset.jsonl'
)
download_file(
    'https://huggingface.co/datasets/Yuchan5386/gpt_pretrain_small/resolve/main/unigram.model?download=true',
    'ko_unigram.model'
)
...
with open("dataset.jsonl", "r", encoding="utf-8") as f:
    for line in f:
        ...
        if "text" in item and item["text"].strip():
            texts.append(f"<start> {item['text'].strip().replace(chr(10), ' ')} <end>")

# 필요하면 샘플 수 제한
train_sentences = texts[:50000]
...
for sentence in train_sentences:
    input_ids = text_to_ids(sentence)
    ...
    targets.append(input_ids[1:] + [pad_id])  # 다음 토큰 예측용
