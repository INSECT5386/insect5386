...
class SimpleFFN(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        up = self.up_proj(x)
        f = self.multi([gate, up])
        return self.down_proj(f)
...
context_vector = SimpleFFN(d_model)(x_pos, training=True)
...
decoder_output = SimpleFFN(d_model)(y_pos, training=True)
decoder_output = GLALayer(d_model)(decoder_output, context_vector)
output = SimpleFFN(d_model)(decoder_output)
